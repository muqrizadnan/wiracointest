...
#if defined(NDEBUG)
# error "Wiracoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7d1621e0aa1e63b0357689152568cdf0eb8a17c187a3537f3995848f4ef0dcc3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wiracoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wiracoin Signed Message:\n";
...
{
    // Wiracoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Wiracoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wiracoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // Wiracoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wiracoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wiracoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf05e4491223a7095c07e97fa05a393f5ac5c6ddc723cf45ccfb2acb9351e9b82");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Boris Johnson is pushing forward for Brexit deal, 23/10/2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1571790915;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085628086;

        if (fTestNet)
        {
            block.nTime    = 1571790897;
            block.nNonce   = 387222724;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7aa35f4dc4ea2ed1dae90da2e49441c7273704fc5c17e3d10d0b79a753999465"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbc, 0xd7 }; // Wiracoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WiracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WiracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WiracoinMiner(CWallet *pwallet)
{
    printf("WiracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wiracoin-miner");
    ...
    try { loop {
        ...
        printf("Running WiracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WiracoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WiracoinMiner, pwallet));
}
